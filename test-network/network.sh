...
function checkPrereqs() {
  ...
  /home/thanhpp/go/src/github.com/thanhpp/fabric-gm/build/bin/peer version > /dev/null 2>&1
  ...
  # docker images
  LOCAL_VERSION=$(/home/thanhpp/go/src/github.com/thanhpp/fabric-gm/build/bin/peer version | sed -ne 's/^ Version: //p')
  DOCKER_IMAGE_VERSION=$(${CONTAINER_CLI} run --rm hyperledger/fabric-tools:amd64-2.4.7-snapshot-cc997a9 peer version | sed -ne 's/^ Version: //p')
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    CA_DOCKER_IMAGE_VERSION=$(${CONTAINER_CLI} run --rm hyperledger/fabric-ca:amd64-2.4.7-snapshot-cc997a9 fabric-ca-client version | sed -ne 's/ Version: //p' | head -1)
...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    which /home/thanhpp/go/src/github.com/thanhpp/fabric-gm/build/bin/cryptogen
    if [ "$?" -ne 0 ]; then
    ...
    set -x
    /home/thanhpp/go/src/github.com/thanhpp/fabric-gm/build/bin/cryptogen generate --config=./organizations/cryptogen/crypto-config-org1.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
    ...
    set -x
    /home/thanhpp/go/src/github.com/thanhpp/fabric-gm/build/bin/cryptogen generate --config=./organizations/cryptogen/crypto-config-org2.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
    ...
    set -x
    /home/thanhpp/go/src/github.com/thanhpp/fabric-gm/build/bin/cryptogen generate --config=./organizations/cryptogen/crypto-config-orderer.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
